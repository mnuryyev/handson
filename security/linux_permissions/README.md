...
Каждый раз, когда администратор выставляет права на файл или директорию, он принимает решение о том, кто и что может сделать в системе. Неправильная настройка этих прав - одна из самых частых причин успешных атак на Linux-системы. Не эксплойты, не zero-day — просто `chmod 777` в нужном месте.

В данной работе разберём Unix-права доступа не в теории, а на практике: создадим изолированную структуру директорий, настроим пользователей, воспроизведём реальные сценарии атак и поймём, зачем в системе вообще существуют SUID, SGID и Sticky bit.
...
В ходе данной работы была построена полная практическая карта Unix-прав доступа: от базового `chmod` до специальных битов SUID, SGID и Sticky. Каждый механизм был проверен в реальных сценариях с несколькими пользователями — именно так права работают в продакшн-системах. Понимание этих механизмов критически важно как для защиты систем, так и для их аудита.
